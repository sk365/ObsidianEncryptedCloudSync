#!/bin/bash
## remove shebang when placing Android file in .shortcuts ! 

## Enter your paths here
local_path=/Users/stefan/Obsidian_Local
cloud_path=/Users/stefan/Dropbox/Obsidian_Cloud

## Define your devices here
this_device=mac
other_device=android

# these flags serve to test if vault is open and when it was opened
flag_this_device="${cloud_path}/private_vault_is_open_"${this_device}".f"
flag_other_device="${cloud_path}/private_vault_is_open_"${other_device}".f"
...
# open vault if closed on this device
	...
	do
		...
		if [ "$pw" = "x" ]; then exit; fi
# remove decrypted notes if existing to avoid conflict
		rm -rf private_notes
# retrieve zip file from cloud and decrypt	
...
# 	move them to the cloud and remove locally 
